...
int main(){
	...
	InitQueue(&free_page);
	/*
	   frame = (frame_ob*)malloc(sizeof(frame_ob)*0x100000);	
	   for(int a=0;a<0x100000;a++){
	   frame[a].frame_entry = (int*)malloc(sizeof(int)*0x400);
	   }

	   page = (page_ob*)malloc(sizeof(page_ob)*USER);
	   for(int a=0;a<USER;a++){
	   page[a].page_entry=(int*)malloc(sizeof(int)*0x400);
	   }
	   page2 = (page_ob*)malloc(sizeof(page_ob)*0x400);
	   for(int a=0;a<0x400;a++){
	   page2[a].page_entry=(int*)malloc(sizeof(int)*0x400);
	   }
	 */
	...
	int count_page2 = counting(&free_page);
			...
			if(count_page2>0x40){
				for(int loop=0;loop<3;loop++){
					...
					for(int index=0;index<0x400;index++){
						...
						if(check_valid == 1){
							int delete_frame = Dequeue(&used_frame);
							frame_count--;

							store_HDD[delete_frame].addr = frame_info[delete_frame].addr;
							store_HDD[delete_frame].pid = frame_info[delete_frame].pid;
							store_HDD[delete_frame].order = frame_info[delete_frame].order;
							for(int z=0; z<0x400; z++){
								store_HDD[delete_frame].data[z] = frame_info[delete_frame].data[z];
								*(frame_ptr+0x400*delete_frame+z)=0;
							}
							Enqueue(&HDD,delete_frame);
			...
			if(state_bit==0){
				...
				if(frame_count>50){
					for(int del=0;del<0x10;del++){
						...
						int find_addr = store_HDD[delete_frame].addr;
						int find_page1_index = (addr&0xFFC00000)>>22;
						int find_page2_index = (addr&0x003FF000)>>12;
						int find_offset = find_addr&0xFFF;
						int find_page1_entry = *(user[store_HDD[delete_frame].order].TTBR + find_page1_index);
						int find_page2_num = (find_page1_entry & 0xFFC00000)>>22;
				...
				int exist_HDD=0;
				int HDD_num=counting(&HDD);
				int check_frame[HDD_num];
				sorting(&HDD,check_frame);

				for(int x=0;x<HDD_num;x++){
					int check_framenum = check_frame[x];
					int check_addr = store_HDD[check_framenum].addr;
					int check_pid = store_HDD[check_framenum].pid;
					int check_order = store_HDD[check_framenum].order;
					if(check_order == pm_order){
						if((addr&0xFFFFF000)==(check_addr&0xFFFFF000)){
							printf("exist in HDD \n");
							int new_frame = Dequeue(&free_frame);
							int backup_frame = find(&HDD,check_framenum);
							frame_num = backup_frame;
							for(int z=0;z<0x400;z++){
								*(frame_ptr+0x400*new_frame+z)= store_HDD[frame_num].data[z];
								frame_info[new_frame].data[z]=store_HDD[frame_num].data[z];
							}
							exist_HDD=1;
							break;
						}
