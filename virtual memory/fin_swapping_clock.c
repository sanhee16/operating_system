...
void do_child(int signo){
	...
	for(int z=0;z<ADDR_NUM;z++){
		...
		if(loop_count<=ADDR_NUM/2){
			u_info[set_order].mem_acc[z] = (rand()+2)%0xFFFFFFFF;
			msg_pm.diff[z]=0;
			msg_pm.w_data[z]=set_data;
			set_data++;
		}
		else{
			...
			if(state_bit==0){
				...
				if(count_frame>15){
					for(int del=0;del<10;del++){
						...
						int find_page1_index = (find_addr&0xFFC00000)>>22;
						int find_page2_index = (find_addr&0x003FF000)>>12;
				...
				for(int x=0;x<HDD_num;x++){
					...
					if(check_order == pm_order){
						if((addr&0xFFFFF000)==(check_addr&0xFFFFF000)){
							...
							printf("[HDD] VA 0x%08x : %d \n",addr,frame_info[new_frame].data[offset/4]);
							printf("[HDD] VA 0x%08x : %d \n",addr,store_HDD[frame_num].data[offset/4]);
			...
			if(state_bit==1){
				...
				if(diff==0){
					*(frame_ptr+0x400*frame_num+offset/4) = write;
					frame_info[frame_num].data[offset/4]=write;
					printf("write data %d \n",*(frame_ptr+0x400*frame_num+offset/4));
				...
				//p_data[a] = *(frame_ptr+0x400*frame_num+offset/4);
			}
		}
	}
	return ;
